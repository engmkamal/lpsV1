(app.controller("CPRSanctionReportViewCtrl", ["$scope", "$http", "$filter", "$location", '$mdDialog', "$rootScope", "$timeout", "dialogService", 'Excel', function ($scope, $http, $filter, $location, $mdDialog, $rootScope, $timeout, dialogService, Excel) {
    // "use strict";
    $scope.PageTitle = "CPR Sanction Admin Report";
    ...
    $scope.GetCPRDetailSearchForm_ClickEvent = function () {
        try {
            ...
            common.preprocessload();
            $http({
                url: "/Report/GetCPRSanctionDataViewByCPRSearchKey",
                method: "POST",
                headers: {
                    "accept": "application/json;odata=verbose",
                    "content-Type": "application/json;odata=verbose"
                ...
                if (response.data.success) {
                    ...
                    $scope.cprSanctionReportlist = response.data.output;
                    $scope.requestCount.totalCount = $scope.cprSanctionReportlist.length;
                    $scope.requestCount.ApprovedCount = ($filter('filter')($scope.cprSanctionReportlist, { requestStatus: "Approved" }, true)).length;
                    $scope.requestCount.RejectCount = ($filter('filter')($scope.cprSanctionReportlist, { requestStatus: "Reject" }, true)).length;
                    $scope.requestCount.InProgressCount = ($filter('filter')($scope.cprSanctionReportlist, { requestStatus: "In Progress" }, true)).length;
                    $scope.requestCount.DraftCount = ($filter('filter')($scope.cprSanctionReportlist, { requestStatus: "Draft" }, true)).length;
