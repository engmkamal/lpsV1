...
import { Component, TemplateRef, ViewChild, WritableSignal, computed, effect, inject, signal } from '@angular/core';
import { AbstractControl, FormBuilder, FormControl, FormGroup, Validators } from '@angular/forms';
import { toSignal } from "@angular/core/rxjs-interop";
import { ActivatedRoute, ResolveEnd, ResolveStart, Route, Router } from '@angular/router';
import { IRouteParam, ITemplateMaster } from '@lps/models';
...
import { NgxLoadingService } from '@lps/service';
...
export class CprinitiationhomeComponent {
  ...
  async ngOnInit(){
    ...
    //this.ngxLoadingService.loading = true;

    //==== get activated route params =====
    this._route.params.subscribe((param: IRouteParam)=>{
      this.routeParam.section = param.section;
    });

    //==== get resolver service data =====
    this._route.data.subscribe(({ masterTemplet }) => {
      // do something with your resolved data ...
      this.masterTemplet = masterTemplet;

      let tempSection = masterTemplet[0].category.keywordText; 
    });


    //==== set loading start ends =========
    ...
    //this.isLoading$.subscribe(loading => this.ngxLoadingService.loading = loading );
    setTimeout(() => {
      //this.isLoading$ = merge(this._showLoaderEvents$, this._hideLoaderEvents$);
      this.isLoading$.subscribe(loading => this.ngxLoadingService.loading = loading );
    }, 5000);
