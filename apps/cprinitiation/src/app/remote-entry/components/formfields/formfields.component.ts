...
import { Component, OnInit, ViewChild, Renderer2, AfterViewInit, AfterViewChecked, EventEmitter, Inject, Input } from '@angular/core';
...
@Component({
  selector: 'lps-formfields',
  templateUrl: './formfields.component.html',
  styleUrl: './formfields.component.scss',
})
export class FormfieldsComponent implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  rowData$!: Observable<any[]>;
    ...
    fldCategory = "inputString";
    
    @Input() fieldObjEmitter!: EventEmitter<any>;
    private fieldObjSubscription!: Subscription;

    parentChildFldRen = 'Hello from the parent component!';
    ...
    async ngOnInit() {
      ...
      const dbListsInfoUrl = "http://localhost:4202/assets/dashboardslistsinfo.ts";
  ...
  onFldCategoryChanged() {
    this.fldCategory = (document.getElementById('fldCategory') as HTMLInputElement)
      ...
      if( this.fldCategory == 'select' ){
        this.rowData = [];
        this.rowData = this.rowDataSelect;
      }
      else if( this.fldCategory == 'multiSelect' ){
        this.rowData = [];
        this.rowData = this.rowDataMultiSelect;
      }
      else if( this.fldCategory == 'radio' ){
        this.rowData = [];
        this.rowData = this.rowDataRadio;
      }
      else if( this.fldCategory == 'date' ){
        this.rowData = [];
        this.rowData = this.rowDataDate;
      }
  ...
  onAddThisField(event: Event){

    console.log("from Parent Componet: " + this.parentChildFldRen);
    const obj = {
      id: 1,
      control: "cif2",
      name: "CIF2",
    ...
    const newObj = [...this.rowData, event];

    this.rowData = [];

    this.rowData = newObj;
    // const emitedFldObj = event;
