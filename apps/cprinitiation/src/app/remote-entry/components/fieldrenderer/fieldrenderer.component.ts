...
import { 
  ...
  ViewEncapsulation, OnChanges, Output 
} from '@angular/core';
...
@Component({
  selector: 'lps-fieldrenderer',
  templateUrl: './fieldrenderer.component.html',
  styleUrl: './fieldrenderer.component.scss',
  encapsulation: ViewEncapsulation.Emulated,
})
export class FieldrendererComponent implements OnInit, OnDestroy, AfterViewInit, AfterViewChecked, ICellRendererAngularComp, OnChanges {
  ...
  @Output() addThisField = new EventEmitter<any>();
  private addFieldSubscription!: Subscription;

  @Input() parentChild!: any; 

  @Output() parentChildChange = new EventEmitter<any>();
  ...
  examples: { [key: string]: { description: string, value: any, schema: Schema } } =
    {
      ...
      metaschema: {
        ...
        value: FieldrendererComponent.schemaExample,
        schema: FieldrendererComponent.metaschema
      }
  };
   ...
   */
    select(key: string) {
      ...
      this.error = '';
      this.set(this.examples[key].schema, this.examples[key].value, "test", "Group Text")
    }
    ...
    set(schema: any, value: any, name:string, groupTitle?:string) {
      ...
      let add
      if (schema.type === 'checkboxGroup')
      add = this.lpsFormrendererHost.viewContainerRef.createComponent(JsonSchemaFormComponent)
      else 
      add = this.lpsFormrendererHost.viewContainerRef.createComponent(JsonSchemaFormComponent)
      ...
      else if (schema.type === 'object' || schema.type === 'checkboxGroup')
  ...
  subscribeToParentEmitter(): void { 
    this.subscription = this.eventEmitter.subscribe((data: string) => { 
      ...
      this.set(parsedData.schema, parsedData.value, parsedData.name);
  ...
  addField(event?:Event){ 
    ...
    this.parentChildChange.emit('updated value from Child !');

    this.addThisField.emit(this.state);

    // const text:any = (event.target as HTMLInputElement).value;
    // this.addFieldSubscription = text.subscribe((data: string) => {       
    //   const fldObj = JSON.parse(data); 
    //   this.fieldObjEmitter.emit(fldObj);
    // });
