...
import { 
  from, 
  groupBy, tap, mergeMap, reduce, map, 
  //filter, catchError, observable, Observable, of, Subject, pipe,
} from 'rxjs';
...
export class SharepointlistService {  
  ...
  public apiUrl = window.location.origin + "/sites/BergerTech"; //uncomment for localhost 
  ...
  async saveListItem(list:any): Promise<any>{
    ...
    const promiseSLI:any = await new Promise((resolve:any, reject:any) => {
